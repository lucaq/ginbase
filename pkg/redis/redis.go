...
func Setup() error {
	RedisConn = &redis.Pool{
		...
		IdleTimeout: global.GINBASE_CONFIG.Redis.IdleTimeout * time.Second,
		Dial: func() (redis.Conn, error) {
			c, err := redis.Dial("tcp", global.GINBASE_CONFIG.Redis.Host)
			if err != nil {
				return nil, err
			}
			if global.GINBASE_CONFIG.Redis.Password != "" {
				if _, err := c.Do("AUTH", global.GINBASE_CONFIG.Redis.Password); err != nil {
					c.Close()
					return nil, err
				}
			}

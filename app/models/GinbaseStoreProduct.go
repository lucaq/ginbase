...
type GinbaseStoreProduct struct {
	...
	ProductCate  *GinbaseStoreCategory `json:"productCate" gorm:"foreignKey:CateId;association_autoupdate:false;association_autocreate:false"`
...
func (GinbaseStoreProduct) TableName() string {
	return "ginbase_store_product"
}

func GetProduct(id int64) GinbaseStoreProduct {
	var product GinbaseStoreProduct
...
// get all
func GetFrontAllProduct(pageNUm int, pageSize int, maps interface{}, order string) (int64, []GinbaseStoreProduct) {
	var (
		total int64
		data  []GinbaseStoreProduct
	)

	db.Model(&GinbaseStoreProduct{}).Where(maps).Count(&total)
	if order == "" {
		order = "id desc"
	}
...
// get all
func GetAllProduct(pageNUm int, pageSize int, maps interface{}) (int64, []GinbaseStoreProduct) {
	var (
		total int64
		data  []GinbaseStoreProduct
	)
...
func AddProduct(m *GinbaseStoreProduct) error {
	var err error
...
func UpdateByProduct(id int64, m *GinbaseStoreProduct) error {
	var err error
	err = db.Model(&GinbaseStoreProduct{}).Where("id = ?", id).Updates(m).Error
	if err != nil {
		return err
	}
...
func OnSaleByProduct(id int64, status int) (err error) {
	...
	err = db.Model(&GinbaseStoreProduct{}).Where("id = ?", id).Update("is_show", isShow).Error
	return
}

func DelByProduct(ids []int64) error {
	var err error
	err = db.Where("id in (?)", ids).Delete(&GinbaseStoreProduct{}).Error
	if err != nil {
		return err
	}

...
func (p *AfterSalesParan) Valid(v *validation.Validation) {
	if vv := v.Required(p.OrderCode, "ginbase-warning"); !vv.Ok {
		vv.Message("单号错误")
		return
	}
	if vv := v.Required(p.ServiceType, "ginbase-warning"); !vv.Ok {
		vv.Message("请选择服务类型")
		return
	}
	if vv := v.Required(p.ProductParamList, "ginbase-warning"); !vv.Ok {

...
import (
	...
	"ginbase/app/params"
	"ginbase/app/service/cart_service"
	cartVo "ginbase/app/service/cart_service/vo"
	orderDto "ginbase/app/service/order_service/dto"
	ordervo "ginbase/app/service/order_service/vo"
	userVO "ginbase/app/service/wechat_user_service/vo"
...
func (d *Order) CancelOrder() error {
	...
	//删除订单
	err = tx.Where("id = ?", order.Id).Delete(&models.GinbaseStoreOrder{}).Error
	if err != nil {
		global.GINBASE_LOG.Error(err)
...
func RegressionStock(tx *gorm.DB, order *ordervo.StoreOrder) error {
	...
	for _, vo := range cartInfoList {
		err = tx.Exec("update ginbase_store_product_attr_value set stock=stock + ?, sales=sales - ?"+
			" where product_id = ? and `unique` = ? and stock >= ?",
			vo.CartNum, vo.CartNum, vo.ProductId, vo.ProductAttrUnique, vo.CartNum).Error
		if err != nil {
			return err
		}
		err = tx.Exec("update ginbase_store_product set stock=stock + ?, sales=sales - ?"+
			" where id = ? and stock >= ?",
...
func (d *Order) OrderComment() error {
	...
	var replys []models.GinbaseStoreProductReply
	for _, data := range d.ReplyParam {
		reply := models.GinbaseStoreProductReply{
			Uid:          d.Uid,
			Oid:          order.Id,
	...
	err = tx.Model(&models.GinbaseStoreProductReply{}).Select("uid", "oid", "product_id",
		"product_score", "service_score", "comment", "pics", "unique").Create(replys).Error
	if err != nil {
		global.GINBASE_LOG.Error(err)
		return errors.New("评价失败-0001")
	}
	err = tx.Model(&models.GinbaseStoreOrder{}).Where("id = ?", order.Id).Update("status", 3).Error
	if err != nil {
		global.GINBASE_LOG.Error(err)
...
func (d *Order) TakeOrder() error {
	...
	//修改订单状态
	err = tx.Model(&models.GinbaseStoreOrder{}).Where("id = ?", order.Id).Update("status", 2).Error
	//增加状态
	err = models.AddStoreOrderStatus(tx, order.Id, "user_take_delivery", "用户已收货")

	//奖励积分
	if order.GainIntegral > 0 {
		err = tx.Exec("update ginbase_user set integral = integral + ?"+
...
//创建订单
func (d *Order) CreateOrder() (*models.GinbaseStoreOrder, error) {
	var err error
	tx := global.GINBASE_DB.Begin()
	defer func() {
		if err != nil {
	...
	err = global.GINBASE_DB.Model(&models.GinbaseUserAddress{}).
		Where("uid = ?", d.Uid).
	...
	storeOrder := &models.GinbaseStoreOrder{
		Uid:            d.Uid,
	...
	err = tx.Model(&models.GinbaseStoreOrder{}).
	...
	//todo 扣积分
	//todo 消费优惠券
	orderInfoList := make([]models.GinbaseStoreOrderCartInfo, 0)
	//减库存加销量
	for _, vo := range cartInfo {
		...
		b, _ := json.Marshal(vo)
		uuid := ksuid.New()
		orderCartInfo := models.GinbaseStoreOrderCartInfo{
			Oid:          storeOrder.Id,
	...
	//保存购物车商品信息
	err = tx.Model(&models.GinbaseStoreOrderCartInfo{}).Create(orderInfoList).Error
	if err != nil {
		return nil, errors.New("订单创建失败-00001")
	}

	//增加状态
	err = models.AddStoreOrderStatus(tx, storeOrder.Id, "ginbase_create_order", "订单生成")
	if err != nil {
...
func (d *Order) GetOrderInfo() (*ordervo.StoreOrder, error) {
	var (
		order *models.GinbaseStoreOrder
	...
	err := global.GINBASE_DB.Model(&models.GinbaseStoreOrder{}).
...
func HandleOrder(order *ordervo.StoreOrder) *ordervo.StoreOrder {
	var (
		orderInfoList []models.GinbaseStoreOrderCartInfo
		cart          cartVo.Cart
		statusDto     orderDto.Status
	)
	global.GINBASE_DB.Model(&models.GinbaseStoreOrderCartInfo{}).Where("oid = ?", order.Id).Find(&orderInfoList)
...
func (d *Order) Check() (map[string]interface{}, error) {
	...
	var order *models.GinbaseStoreOrder
	error := global.GINBASE_DB.Model(&models.GinbaseStoreOrder{}).
		Where("`unique` = ?", d.Key).
...
func (d *Order) ComputeOrder() (*ordervo.Compute, error) {
	global.GINBASE_LOG.Info(d.ComputeParam)
	var (
		payPostage     = 0.00
		couponPrice    = 0.00
		deductionPrice = 0.00
...
func (d *Order) ConfirmOrder() (*ordervo.ConfirmOrder, error) {
	...
	//获取默认地址
	global.GINBASE_DB.Model(&models.GinbaseUserAddress{}).
		Where("uid = ?", d.Uid).
		Where("is_default = ?", 1).
		First(&userAddress)
...
func getOrderPriceGroup(cartInfo []cartVo.Cart, userAddress *models.GinbaseUserAddress) orderDto.PriceGroup {
	var (
...
func (d *Order) GetAll() vo.ResultList {
	...
	var (
		orderInfoList []models.GinbaseStoreOrderCartInfo
		cart          cartVo.Cart
		newList       []models.GinbaseStoreOrder
	)
	for _, order := range list {
		...
		//global.GINBASE_LOG.Info(order.CartInfo)
...
func (d *Order) Deliver() error {
	...
	var express models.GinbaseExpress
	err := global.GINBASE_DB.Model(&models.GinbaseExpress{}).Where("name = ?", d.M.DeliveryName).First(&express).Error
	if err != nil {
		return errors.New("请先添加快递公司")
	}
	global.GINBASE_LOG.Info(d.M.DeliveryId)
	d.M.Status = 1

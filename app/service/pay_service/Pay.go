	...
	case "weixin":
		if from == "pc" {
			...
			bm.Set("nonce_str", util.RandomString(32)).
				Set("body", "ginbase-go pc支付").
				Set("out_trade_no", orderId).
				Set("total_fee", orderInfo.PayPrice*100).
				Set("spbill_create_ip", "127.0.0.1").
				Set("notify_url", "https://www.ginbase.co/notify").
...
func YuePay(orderId string, uid int64) error {
	...
	global.GINBASE_LOG.Info(userInfo.NowMoney, orderInfo.PayPrice)
	if userInfo.NowMoney < orderInfo.PayPrice {
		return errors.New("余额不足")
	}
	err = tx.Exec("update ginbase_user set now_money=now_money - ?"+
...
func PaySuccess(tx *gorm.DB, orderId, payType string) error {
	...
	//修改订单状态
	updateOrder := &models.GinbaseStoreOrder{
		Paid:    orderEnum.PAY_STATUS_1,
		PayType: payType,
		PayTime: time.Now(),
	}
	err = tx.Model(&models.GinbaseStoreOrder{}).Where("order_id = ?", orderId).Updates(updateOrder).Error
	...
	//增加用户购买次数
	err = tx.Exec("update ginbase_user set pay_count = pay_count + 1"+
